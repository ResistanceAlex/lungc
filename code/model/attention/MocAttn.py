import numpy as np
from typing import Any, Callable, Optional, Callable, Union, Tuple, Any
...
if __name__ == "__main__":
    ...
    # 创建MocAttn模块
    moc_att = MoCAttention(55,55).to(device)

    # 创建一个随机输入张量 (batch_size=1, channels=64, height=32, width=32)
    dummy_img = torch.randn(1, 256, 55, 55).cuda()  # 假设输入尺寸
    
    dummy_img = dummy_img.permute(2, 3, 1, 0)
    print(dummy_img.shape)
    
    # 前向传播
    output = moc_att(dummy_img).permute(3, 2, 0, 1)
